...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-sampler-route-contract",
        },
        "buffers": [{
            "uri": "sampler-route.bin",
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports decoded texture sampler state.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "sampler-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "sampler-route.gltf"
    ...
    print("renderer_probe_sampler_route_verified=true")
    return 0
