#!/usr/bin/env python3
"""Verifies the double-sided probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_double_sided_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_double_sided_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify double-sided route smoke drift cases.")
    ...
    print("renderer_probe_double_sided_route_contract_verified=true")
    return 0
