...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-orthographic-camera-route-contract",
        },
        "buffers": [{
            "uri": "orthographic-camera-route.bin",
        ...
        "materials": [{
            "name": "OrthographicCameraMat",
            "pbrMetallicRoughness": {
                "baseColorFactor": [0.85, 0.85, 0.85, 1.0],
            },
        }],
        "meshes": [{
            "name": "OrthographicCameraMesh",
            "primitives": [{
                "attributes": {
                    "POSITION": 0,
                    "TEXCOORD_0": 1,
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports orthographic camera routing.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "orthographic-camera-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "orthographic-camera-route.gltf"
    ...
    print("renderer_probe_orthographic_camera_route_verified=true")
    return 0
