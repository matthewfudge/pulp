...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-resource-floor-route-contract",
        },
        "buffers": [{
            "uri": "resource-floor-route.bin",
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports native resource floor telemetry.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "resource-floor-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "resource-floor-route.gltf"
    ...
    print("renderer_probe_resource_floor_route_verified=true")
    return 0
