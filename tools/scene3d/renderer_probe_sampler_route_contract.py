#!/usr/bin/env python3
"""Verifies the texture sampler probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_sampler_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_sampler_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify texture sampler route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        clamp_s_lost = dict(base_fields)
        clamp_s_lost["texture_sampler_clamp_s"] = "false"
        write_probe(probe, clamp_s_lost)
        expect_case(
            "sampler-clamp-s-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "texture_sampler_clamp_s: expected 'true', got 'false'",
            errors,
        )

        clamp_t_lost = dict(base_fields)
        clamp_t_lost["texture_sampler_clamp_t"] = "false"
        write_probe(probe, clamp_t_lost)
        expect_case(
            "sampler-clamp-t-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "texture_sampler_clamp_t: expected 'true', got 'false'",
            errors,
        )
    ...
    print("renderer_probe_sampler_route_contract_verified=true")
    return 0
