...
// a plain JS function cannot sit in the module's indirect function table.
// `WebAssembly.Function` (the type-reflection proposal) wraps a JS callback into
// a real funcref. It is available in Node behind
// `--experimental-wasm-type-reflection` and in recent Chrome. So NO compiled
// C++ host shim is needed — the host vtable is synthesized from JS here.
//
// This is the engine behind the Node host runner (wclap_host_runner.mjs) and is
// structured so a browser/AudioWorklet host can reuse the same struct
// marshalling. All CLAP struct offsets below are for the wasm32 ABI and pinned
// to the CLAP 1.2.x layout the WebCLAP module is built against.
...
export class WebClapHost {
  ...
  async instantiate(wasmBytesOrModule) {
    if (typeof WebAssembly.Function !== "function") {
      throw new Error(
        "WebClapHost needs WebAssembly.Function (Node: --experimental-wasm-type-reflection)");
    }
  ...
  _addFn(sig, fn) {
    ...
    tbl.grow(1);
    tbl.set(idx, new WebAssembly.Function(sig, fn));
    return idx;
  }
...
export class WebClapPlugin {
  ...
  process(input, frames, { paramEvents = [] } = {}) {
    ...
    // in_events: build the param-value events, expose via size()/get().
    const eventPtrs = paramEvents.map(({ id, value }) => {
      const e = h.ex.malloc(PARAM_EVENT_SIZE);
    ...
    const inEvents = h.ex.malloc(12);
    h.setU32(inEvents + 0, 0);
    h.setU32(inEvents + 4, h._addFn({ parameters: ["i32"], results: ["i32"] },
      () => eventPtrs.length));
    h.setU32(inEvents + 8, h._addFn({ parameters: ["i32", "i32"], results: ["i32"] },
      (_ctx, i) => eventPtrs[i] ?? 0));
    const outEvents = h.ex.malloc(8);
    h.setU32(outEvents + 0, 0);
    h.setU32(outEvents + 4, h._addFn({ parameters: ["i32", "i32"], results: ["i32"] },
      () => 0));

    // clap_process_t (40 bytes).
    const proc = h.ex.malloc(40);
    ...
    h.setU32(proc + 32, inEvents); h.setU32(proc + 36, outEvents);
