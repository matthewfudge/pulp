...
def make_scene_bin():
    ...
    alpha_png = make_png_rgba(2, 1, [
        220, 80, 80, 0,
        220, 80, 80, 255,
    ])
    alpha_image_view = add_view(alpha_png)

    return b"".join(chunks), views, {
        "position": position_view,
        "texcoord": texcoord_view,
        "indices": index_view,
        "alpha_image": alpha_image_view,
    }
...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-alpha-mask-route-contract",
        },
        "buffers": [{
            "uri": "alpha-mask-route.bin",
        ...
        "images": [
            {"bufferView": ids["alpha_image"], "mimeType": "image/png", "name": "alpha"},
        ],
        "textures": [
            {"source": 0},
        ],
        ...
        "nodes": [{"name": "AlphaMaskNode", "mesh": 0}],
        "scenes": [{"nodes": [0]}],
        "scene": 0,
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports alpha-mask texture cutoff.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "alpha-mask-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "alpha-mask-route.gltf"
    ...
    print("renderer_probe_alpha_mask_route_verified=true")
    return 0
