#!/usr/bin/env python3
"""Verifies the vertex-color probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_vertex_color_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_vertex_color_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify vertex-color route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        vertex_color_lost = dict(base_fields)
        vertex_color_lost["vertex_color_applied"] = "false"
        write_probe(probe, vertex_color_lost)
        expect_case(
            "vertex-color-route-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "vertex_color_applied: expected 'true', got 'false'",
            errors,
        )
    ...
    print("renderer_probe_vertex_color_route_contract_verified=true")
    return 0
