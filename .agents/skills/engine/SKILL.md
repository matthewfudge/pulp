...
- **Windows:** no clean dev `libnode` package. Either build Node
  ...
  `V8_LIBRARY_PATH`, or use the sealed v8-builder Windows drop-in (below).
...
Wiring: set `PULP_V8BUILDER_ROOT` (the unpacked `v8-win-x64` root with
`include/`, `lib/v8.dll.lib`, `manifest.json`) plus
`PULP_V8_WIN_LIBCXX_INCLUDE` and `PULP_V8_WIN_LIBCXX_LIB`.
`core/view/CMakeLists.txt` derives `V8_INCLUDE_DIR/LIB_DIR/LIBRARY_PATH`
from the sealed root and calls `pulp_v8_windows_apply_abi(pulp-view-script)`
(no-op off Windows). Proven: choc V8 consumer init + `evaluateExpression
