...
Last live check: 2026-05-01 21:32:05 EDT.
...
| `feature/mkdocs-hooks-coverage-643` | `900faf7b` | #643 tooling tranche for `tools/mkdocs_hooks.py` paths | `tools/scripts/test_mkdocs_hooks.py` | Rebased cleanly onto `origin/main` at `ef430385`; `python3 tools/scripts/test_mkdocs_hooks.py` passed with 10 tests; `git diff --check origin/main...HEAD` passed; tracked status clean with diff limited to `tools/scripts/test_mkdocs_hooks.py`. Local `python3 tools/scripts/run_python_coverage.py --pattern tools/scripts/test_mkdocs_hooks.py` is blocked because system Python lacks `coverage.py >= 7.10`, and `python3 -m pytest tools/scripts/test_mkdocs_hooks.py -v` is blocked because `pytest` is not installed. Earlier uv-backed focused coverage for this tranche reported 100% for `tools/mkdocs_hooks.py`. | Ready to queue when a slot opens; push/open with `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
