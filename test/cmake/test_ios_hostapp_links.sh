...
# `PulpSineSynth_HostApp` produces the .app shell but skips the
# embed step (the embed lives on `PulpSineSynth_HostApp_Embed`), so
# the user gets a HostApp without an embedded AUv3 extension.
set +e
if [[ ${#timeout_cmd[@]} -gt 0 ]]; then
