...
| `feature/widget-bridge-drop-font-coverage-493` | `204b8fc9` | #493 view tranche for `core/view/src/widget_bridge.cpp` JS drop callback escaping and `loadFont` existence paths | `test/test_widget_bridge.cpp` | Created from current `origin/main`; `cmake --build build --target pulp-test-widget-bridge -j4`; `./build/test/pulp-test-widget-bridge "[dnd],[font]"` passed 12 assertions in 2 test cases; `ctest --test-dir build -R 'widget-bridge\|WidgetBridge' --output-on-failure` passed 82/82; skill-sync report; version-bump report; `git diff --check`. Local diff coverage was intentionally skipped because the branch is test-only and local disk is about 2.9 GiB free. | Hold local-only while current Namespace queue is saturated; when capacity drains, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`, then dispatch Namespace with `shipyard cloud run build feature/widget-bridge-drop-font-coverage-493 --require-sha HEAD`. |
