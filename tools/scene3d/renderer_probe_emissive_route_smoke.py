...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-emissive-route-contract",
        },
        "extensionsUsed": ["KHR_materials_emissive_strength"],
        "buffers": [{
            "uri": "emissive-route.bin",
        ...
        "nodes": [{"name": "EmissiveNode", "mesh": 0}],
        "scenes": [{"nodes": [0]}],
        "scene": 0,
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports emissive material state.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "emissive-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "emissive-route.gltf"
    ...
    print("renderer_probe_emissive_route_verified=true")
    return 0
