...
| `queue_lifecycle.py` | Locked queue enqueue/load/claim/finalize lifecycle helpers, active-target/target-state update persistence, CLI queue bump/cancel persistence, stale-validator queue collection, running-job reconciliation action application, drain loop coordination, and job-wait polling that coordinate queue lock acquisition, running-job reconciliation, queue persistence, runner claim records, completion trimming, post-finalize cleanup, and wait-loop callbacks through injected dependencies. | Enqueue/drain policy, runner-info persistence, result creation, target execution, or user-facing CLI output. |
