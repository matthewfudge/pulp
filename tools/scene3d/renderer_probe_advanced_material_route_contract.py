#!/usr/bin/env python3
"""Verifies the advanced-material route smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_advanced_material_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_advanced_material_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify advanced-material route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        advanced_lost = dict(base_fields)
        advanced_lost["advanced_material_extension_deferred"] = "false"
        write_probe(probe, advanced_lost)
        expect_case(
            "advanced-material-deferral-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "advanced_material_extension_deferred: expected 'true', got 'false'",
            errors,
        )

        double_sided_lost = dict(base_fields)
        double_sided_lost["double_sided_material_applied"] = "false"
        write_probe(probe, double_sided_lost)
        expect_case(
            "double-sided-route-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "double_sided_material_applied: expected 'true', got 'false'",
            errors,
        )

        texture_leak = dict(base_fields)
        texture_leak["texture_decoded"] = "true"
        write_probe(probe, texture_leak)
        expect_case(
            "texture-decode-leak",
            run_smoke(args.route_smoke, probe),
            1,
    ...
    print("renderer_probe_advanced_material_route_contract_verified=true")
    return 0
