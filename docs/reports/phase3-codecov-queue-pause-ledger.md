...
Last live check: 2026-05-01 19:49:38 EDT.
...
- GitHub Actions pressure: #1231 and #1232 have PR-event Build/Coverage
  checks active or queued on Namespace-backed lanes and no failed checks.
  Main changed after #1230 merged, so re-check mergeability before merging
  either remaining PR.
...
- Local-only progress: `pulp-lcov-cobertura-extra-643` advanced to
  `8cb1d812b7c2` with zero-hit line, demangler, missing `c++filt`, and
...
| `local/phase3-cmajor-external-extra-643` | `371f0562` | #643 tooling tranche for `tools/scripts/cmajor_external.py` edges | `tools/scripts/test_cmajor_external_extra.py` | Rebased cleanly onto `origin/main` at `a12411e4`; `python3 -m unittest tools.scripts.test_cmajor_external tools.scripts.test_cmajor_external_extra` passed with 13 tests; focused coverage passed and reported 100% for `tools/scripts/cmajor_external.py` with 119 statements, 0 misses, 32 branches, and 0 partials; `python3 tools/scripts/cmajor_external.py doctor --patch examples/cmajor-gain/CmajorGain.cmajorpatch` passed and reported expected missing external `cmaj`; `git diff --check origin/main...HEAD` passed; final tracked status clean. System Python lacked coverage, so an existing Pulp venv was used; the subprocess-heavy base test emitted one no-data warning, but the combined selected run completed with exit code 0 and produced the target 100% row. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
...
| `local/phase3-merge-cobertura-extra-643` | `f25ed82a` | #643 tooling tranche for `tools/scripts/merge_cobertura.py` edges | `tools/scripts/test_merge_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `a12411e4`; `.venv/bin/python tools/scripts/test_merge_cobertura.py` passed with 14 tests; `.venv/bin/python tools/scripts/test_merge_cobertura_extra.py` passed with 3 tests; `.venv/bin/python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_merge_cobertura.py --pattern tools/scripts/test_merge_cobertura_extra.py` passed and reported 100% for `tools/scripts/merge_cobertura.py` with 121 statements, 0 misses, 48 branches, and 0 partials; docs-sync report passed; `uv run --with PyYAML python tools/scripts/test_codecov_config.py` passed after the worktree venv lacked PyYAML; `git diff --check origin/main...HEAD` passed; final tracked status clean. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
| `local/phase3-lcov-cobertura-extra-643` | `8cb1d812` | #643 tooling tranche for `tools/scripts/lcov_cobertura.py` zero-hit, demangler, missing-tool, and entrypoint edges | `tools/scripts/test_lcov_cobertura_extra.py` | Rebased cleanly onto `origin/main` at `a12411e4`; `python3 tools/scripts/test_lcov_cobertura.py` passed with 8 tests; `python3 tools/scripts/test_lcov_cobertura_extra.py` passed with 11 tests; `uv run --with 'coverage>=7.10' python tools/scripts/run_python_coverage.py --pattern tools/scripts/test_lcov_cobertura.py --pattern tools/scripts/test_lcov_cobertura_extra.py` passed and reported 100% for `tools/scripts/lcov_cobertura.py` with 243 statements, 0 misses, 74 branches, and 0 partials; coverage XML existed and target filename `tools/scripts/lcov_cobertura.py` reported line-rate 1 and branch-rate 1; docs-sync report passed; `git diff --check origin/main...HEAD` passed; final tracked status clean. System Python lacked coverage, so `uv run` was used. | Hold local-only while current Namespace batch drains; when capacity returns, rename/push as a feature branch, run `shipyard pr --skip-target mac --skip-target ubuntu --skip-target windows`; use PR-event Namespace checks unless a targeted diagnostic dispatch is needed. |
