...
#   1. Run the Node + esbuild transform on a JSX fixture.
#   2. Build the JSX runtime test target.
#   3. Run the C++ smoke test (asserts >9 IR nodes + Chainer text).
#   4. Optionally render through pulp-screenshot for a visual artifact.
#
...
# ─── 5. Optionally render a screenshot ──────────────────────────────────
