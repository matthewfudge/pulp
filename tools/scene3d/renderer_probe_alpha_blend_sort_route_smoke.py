...
def make_scene_bin():
    ...
    front_position_view = add_view(append_f32([
        -1.0, -1.0, 0.35,
        1.0, -1.0, 0.35,
        0.0, 1.0, 0.35,
    ]), 34962)
    back_position_view = add_view(append_f32([
        -1.0, -1.0, -0.35,
        1.0, -1.0, -0.35,
        0.0, 1.0, -0.35,
    ]), 34962)
    ...
    return b"".join(chunks), views, {
        "front_position": front_position_view,
        "back_position": back_position_view,
        "texcoord": texcoord_view,
        "indices": index_view,
    }


def position_accessor(buffer_view, z):
    return {
...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-alpha-blend-sort-route-contract",
        },
        "buffers": [{
            "uri": "alpha-blend-sort-route.bin",
            "byteLength": byte_length,
        }],
        "bufferViews": views,
        "accessors": [
            position_accessor(ids["front_position"], 0.35),
            position_accessor(ids["back_position"], -0.35),
            {
        ...
        "materials": [
            {
                ...
                "pbrMetallicRoughness": {
                    "baseColorFactor": [1.0, 0.05, 0.05, 0.55],
                },
                "alphaMode": "BLEND",
                "doubleSided": True,
            },
            {
                "name": "BlueBlendBack",
                "pbrMetallicRoughness": {
                    "baseColorFactor": [0.05, 0.05, 1.0, 0.55],
                },
                "alphaMode": "BLEND",
                "doubleSided": True,
            },
        ...
        "nodes": [{"name": "AlphaBlendSortRouteNode", "mesh": 0}],
        "scenes": [{"nodes": [0]}],
        "scene": 0,
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports sorted alpha-blend layers.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "alpha-blend-sort-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "alpha-blend-sort-route.gltf"
        ...
        require(values.get("primitive_count") == "2",
                f"primitive_count: expected '2', got {values.get('primitive_count')!r}")
    ...
    print("renderer_probe_alpha_blend_sort_route_verified=true")
    return 0
