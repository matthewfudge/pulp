#!/usr/bin/env python3
"""Verifies the directional-light probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_directional_light_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_directional_light_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify directional-light route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        directional_lost = dict(base_fields)
        directional_lost["directional_light_applied"] = "false"
        write_probe(probe, directional_lost)
        expect_case(
            "directional-light-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "directional_light_applied: expected 'true', got 'false'",
            errors,
        )

        normals_lost = dict(base_fields)
        normals_lost["geometry_normals_applied"] = "false"
        write_probe(probe, normals_lost)
        expect_case(
        ...
        transform_leak = dict(base_fields)
        transform_leak["directional_light_transform_applied"] = "true"
        write_probe(probe, transform_leak)
        expect_case(
            "directional-transform-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "directional_light_transform_applied: expected 'false', got 'true'",
            errors,
        )
        ...
        spot_leak = dict(base_fields)
        spot_leak["spot_light_applied"] = "true"
        write_probe(probe, spot_leak)
        expect_case(
            "spot-light-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "spot_light_applied: expected 'false', got 'true'",
            errors,
        )
    ...
    print("renderer_probe_directional_light_route_contract_verified=true")
    return 0
