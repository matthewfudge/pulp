...
float Label::intrinsic_width() const {
    ...
    // size. The earlier "Inter" hardcode under-reserved width for any
    // Label whose painter draws in a different family — so the imported
    // Chainer JSX hit "XOVER → lo_freq" clipping to "XOVER → lo_fre"
    // because Yoga reserved Inter's width but the painter drew Plex
    // Mono's. Mirror paint()'s exact logic (line ~378): use
    // font_family_ if set, else fall through to the inheritable
    // cascade, else default to "Inter".
    std::string effective_family = font_family_;
    if (effective_family.empty()) {
        if (auto inh = inheritable_font_family(); inh.has_value())
            effective_family = inh.value();
