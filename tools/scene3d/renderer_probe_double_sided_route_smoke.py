...
def make_scene_gltf(byte_length, views, ids):
    return {
        "asset": {
            "version": "2.0",
            "generator": "pulp-renderer3d-double-sided-route-contract",
        },
        "buffers": [{
            "uri": "double-sided-route.bin",
        ...
        "materials": [{
            ...
            "pbrMetallicRoughness": {
                "baseColorFactor": [1.0, 0.35, 0.15, 1.0],
            },
            "doubleSided": True,
        }],
        "meshes": [{
        ...
        "nodes": [{"name": "DoubleSidedNode", "mesh": 0}],
        "scenes": [{"nodes": [0]}],
        "scene": 0,
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify renderer probe reports double-sided material state.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (temp_path / "double-sided-route.bin").write_bytes(bin_bytes)
        scene_path = temp_path / "double-sided-route.gltf"
    ...
    print("renderer_probe_double_sided_route_verified=true")
    return 0
