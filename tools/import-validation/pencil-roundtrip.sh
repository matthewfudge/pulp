#!/usr/bin/env bash
# Phase 6.6.6 Pencil runtime-import validation harness.
#
...
if [[ -n "${PULP_PENCIL_FIXTURE:-}" ]]; then
  FIXTURE="$PULP_PENCIL_FIXTURE"
elif [[ -f "$DEFAULT_PLANNING_FIXTURE" ]]; then
  FIXTURE="$DEFAULT_PLANNING_FIXTURE"
else
  FIXTURE="$DEFAULT_TEST_FIXTURE"
fi
BUILD_DIR="${PULP_BUILD_DIR:-$PULP_DIR/build-pencil-import}"
BUILD_TYPE="${PULP_BUILD_TYPE:-Debug}"
BUILD_JOBS="${PULP_BUILD_JOBS:-1}"
REFERENCE="${PULP_PENCIL_REFERENCE:-$PULP_DIR/planning/screenshots/REFERENCE-pencil-gain-stage-card.png}"
OUT="${PULP_PENCIL_OUT:-$PULP_DIR/planning/screenshots/pencil-gain-stage-card-latest.png}"
...
usage() {
  cat <<'EOF'
Usage:
  tools/import-validation/pencil-roundtrip.sh [--parser-only] [--skip-build]
  tools/import-validation/pencil-roundtrip.sh --coverage
  tools/import-validation/pencil-roundtrip.sh --reference path/to/reference.png
...
Env:
  ...
  PULP_PENCIL_FIXTURE     Pencil TSX fixture path (defaults to planning fixture,
                          falls back to test fixture when planning is unavailable)
  PULP_PENCIL_REFERENCE   reference screenshot path
  PULP_PENCIL_OUT         candidate screenshot path
...
[[ -f "$FIXTURE" ]] || { red "missing Pencil fixture: $FIXTURE"; exit 2; }
...
TMP_SCRIPT="$(mktemp "${TMPDIR:-/tmp}/pulp-pencil-runtime.XXXXXX.js")"
trap 'rm -f "$TMP_SCRIPT"' EXIT
PULP_PENCIL_RUNTIME_JS_OUT="$TMP_SCRIPT" \
  "$DESIGN_IMPORT_REACT_RUNTIME_TEST" 'parse_pencil_react runtime bundle materializes with host React shim'
...
"$SCREENSHOT_BIN" \
  ...
  --height 360 \
  --theme dark
green "captured Pencil runtime render: $OUT"

if [[ ! -f "$REFERENCE" ]]; then
  yellow "reference screenshot not found: $REFERENCE"
  yellow "Phase 6.6.6 screenshot diff skipped; use the captured render as the reference once reviewed."
  exit 77
fi
