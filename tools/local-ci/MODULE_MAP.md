...
| `runner_state.py` | Runner-info JSON persistence, runner PID liveness checks, stale runner-info cleanup under the drain lock, and current-runner stale-running selection. | Queue mutation, target execution, runner active-target payload policy, or user-facing CLI output. |
