#!/usr/bin/env python3
"""Verifies the resource-floor route smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_resource_floor_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_resource_floor_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify resource-floor route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for key, name in [
            ("depth_target_allocated", "depth-target-drift"),
            ("color_target_allocated", "color-target-drift"),
            ("vertex_buffer_uploaded", "vertex-buffer-drift"),
            ("index_buffer_uploaded", "index-buffer-drift"),
            ("uniform_buffer_uploaded", "uniform-buffer-drift"),
        ]:
            fields = dict(base_fields)
            fields[key] = "false"
            write_probe(probe, fields)
            expect_case(
                name,
                run_smoke(args.route_smoke, probe),
                1,
                f"{key}: expected 'true', got 'false'",
                errors,
            )

        metallic_lost = dict(base_fields)
        metallic_lost["metallic_roughness_factor_applied"] = "false"
        write_probe(probe, metallic_lost)
        expect_case(
            "metallic-roughness-factor-drift",
            run_smoke(args.route_smoke, probe),
            1,
    ...
    print("renderer_probe_resource_floor_route_contract_verified=true")
    return 0
