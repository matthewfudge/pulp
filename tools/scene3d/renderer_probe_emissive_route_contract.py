#!/usr/bin/env python3
"""Verifies the emissive-material probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_emissive_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_emissive_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify emissive route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        emissive_factor_lost = dict(base_fields)
        emissive_factor_lost["emissive_factor_applied"] = "false"
        write_probe(probe, emissive_factor_lost)
        expect_case(
            "emissive-factor-route-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "emissive_factor_applied: expected 'true', got 'false'",
            errors,
        )

        emissive_strength_lost = dict(base_fields)
        emissive_strength_lost["emissive_strength_applied"] = "false"
        write_probe(probe, emissive_strength_lost)
        expect_case(
            "emissive-strength-route-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "emissive_strength_applied: expected 'true', got 'false'",
            errors,
        )
        ...
        emissive_texture_leak = dict(base_fields)
        emissive_texture_leak["emissive_texture_applied"] = "true"
        write_probe(probe, emissive_texture_leak)
        expect_case(
            "emissive-texture-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "emissive_texture_applied: expected 'false', got 'true'",
            errors,
        )

        unlit_leak = dict(base_fields)
        unlit_leak["unlit_material_applied"] = "true"
        write_probe(probe, unlit_leak)
        expect_case(
            "unlit-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "unlit_material_applied: expected 'false', got 'true'",
            errors,
        )
    ...
    print("renderer_probe_emissive_route_contract_verified=true")
    return 0
