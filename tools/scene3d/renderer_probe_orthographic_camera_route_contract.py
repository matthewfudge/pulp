#!/usr/bin/env python3
"""Verifies the orthographic camera probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_orthographic_camera_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_orthographic_camera_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify orthographic camera route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        expect_case(
            "valid-orthographic-camera-route",
        ...
        orthographic_lost = dict(base_fields)
        orthographic_lost["orthographic_camera_applied"] = "false"
        write_probe(probe, orthographic_lost)
        expect_case(
            "orthographic-camera-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "orthographic_camera_applied: expected 'true', got 'false'",
            errors,
        )

        perspective_leak = dict(base_fields)
        perspective_leak["perspective_camera_applied"] = "true"
        write_probe(probe, perspective_leak)
        expect_case(
            "perspective-camera-leak",
            run_smoke(args.route_smoke, probe),
            1,
        ...
        camera_deferred["camera_depth_range_deferred"] = "true"
        write_probe(probe, camera_deferred)
        expect_case(
            "camera-deferred-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "camera_depth_range_deferred: expected 'false', got 'true'",
            errors,
        )
    ...
    print("renderer_probe_orthographic_camera_route_contract_verified=true")
    return 0
