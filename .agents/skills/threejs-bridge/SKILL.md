...
These failures all present as "demo says `status: 'ready'` but the canvas is solid black/blank" — Three.js's `renderer.init()` uses `new Promise(async (resolve, reject) => {…})` which silently swallows any throw inside the executor, so errors do NOT propagate as rejections. Whenever you see a black canvas, suspect one of these:
...
4. **iOS AUv3 / JSC: canvas swapchain alpha=0** (fixed in PR #3228, 2026-05-31 — keep this gotcha documented). Three.js's WebGPURenderer composite shader writes `alpha=0` for the canvas-targeted pass (its tonemap only computes RGB). With default `writeMask=All` + JS-supplied `loadOp=load`, the canvas destination alpha stays at 0 and SkiaCanvas composites `src.alpha=0` against the canvasCard CSS background — only the CSS bg shows through. Fix in `__gpuQueueDrawBufferedImpl`: when `target_canvas_state != nullptr`, set `color_target.writeMask = R|G|B` (no alpha) AND force `color_attachment.loadOp = Clear` with `clearValue.a = 1.0`. iOS-specific symptom; macOS V8 demo doesn't hit it because the canvas swapchain's initial alpha is 1, not 0.

5. **iOS AUv3 / JSC: WebGPURenderer NodeBuilder generates `@location(0)=normal, @location(1)=position`** — REVERSED from the GL convention. Do NOT assume position-first when reading the WGSL. The shim is correct as long as it's slot-authoritative (`pass.SetVertexBuffer(slot, buffer)` based on what JS sent at each slot). Verify slot→shaderLocation mapping via a one-shot `cube-vb` log in `__gpuQueueDrawBufferedImpl` if you see the cube collapse to nothing.

6. **iOS AUv3 / JSC: cube renders blank despite all of (1)–(5) being correct** (still open in #3217, debug branch `debug/ios-d3c-uniform-data-trace`). All bridge counters report 100% success; canvas is opaque (after #3228); HDR target write+read pointer match; no `__phase13BufferedSkips` events. JSC executes Three.js's vertex shader but produces zero fragments into the HDR target. NEW failure mode not yet root-caused. Per-mesh uniform layout = `mat3x3 normalMatrix + f32 + mat4 modelViewMatrix` = 128B; we verified the first 16 bytes (normalMatrix col 0) ARE valid floats matching expected unit-vector-ish data, so the gross-data path looks right. Suspect a JSC-specific quirk in how Three.js's uniform struct or buffer mapping path differs from V8. Right next step: diff macOS-V8 known-good (`examples/threejs-native-demo`) vs iOS-JSC-blank execution at the createPipeline + queue.submit level — not freelance shim hunting.
