...
| `queue_lifecycle.py` | Locked queue load/claim/finalize lifecycle helpers that coordinate queue lock acquisition, running-job reconciliation, queue persistence, runner claim records, completion trimming, and post-finalize cleanup through injected callbacks. | Enqueue/drain policy, runner-info persistence, result creation, target execution, or user-facing CLI output. |
...
| Queue orchestration | Remaining enqueue/drain locking, queue mutation around extracted helpers, runner-state wrapper calls, drain/wait loops, and CLI-facing queue updates. | later queue modules |
