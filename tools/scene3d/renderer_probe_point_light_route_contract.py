#!/usr/bin/env python3
"""Verifies the point-light probe smoke rejects telemetry drift."""
...
def load_route_smoke(path):
    spec = importlib.util.spec_from_file_location(
        "renderer_probe_point_light_route_smoke", path)
...
def expect_case(name, result, expected_code, expected_text, errors):
    ...
    print(f"renderer_probe_point_light_route_contract_case={name}")


def main():
    parser = argparse.ArgumentParser(
        description="Verify point-light route smoke drift cases.")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        point_lost = dict(base_fields)
        point_lost["point_light_applied"] = "false"
        write_probe(probe, point_lost)
        expect_case(
            "point-light-drift",
            run_smoke(args.route_smoke, probe),
            1,
            "point_light_applied: expected 'true', got 'false'",
            errors,
        )
        ...
        point_deferred = dict(base_fields)
        point_deferred["point_light_deferred"] = "true"
        write_probe(probe, point_deferred)
        expect_case(
            "point-deferred-leak",
            run_smoke(args.route_smoke, probe),
            1,
            "point_light_deferred: expected 'false', got 'true'",
            errors,
        )
    ...
    print("renderer_probe_point_light_route_contract_verified=true")
    return 0
